...
| [`ihris-use-cases`](library/ihris-use-cases/) | the 2009 use-case model of Common, Manage, Qualify and Plan | 138 use cases, 36 requirements; 15 actors declared as 14 roles (the two "Any User" actors are one), each linked to the use cases it is primary actor on, 2 named people as opaque actors; crosswalk to 4.3.3 forms |
...
| `glossary/` | the glossary: one SKOS concept scheme per source (`folio-glossary/v1`, generated by `src/tools/build_glossary.py`), on the site at [glossary/](https://litlfred.github.io/ihris/glossary/) |
...
Every term extracted, as W3C SKOS (folio-assistant core's `folio-glossary/v1`), in [`glossary/`](glossary/): 1725 terms in 35 schemes. The toolkit's 33 technical terms (authored, verbatim); the 2009 use-case reports, whose summary names a glossary that none of them holds (no terms, none invented); and 33 iHRIS 4.3.3 code lists (1692 terms: 541 ISCO-88 groups authored with their shipped definitions, the rest candidates). 245 countries and 143 currencies link by `exactMatch` to the EU Publications Office authority tables, from the verified ISO ConceptMaps only. Searchable on the site's [glossary](https://litlfred.github.io/ihris/glossary/) and [search](https://litlfred.github.io/ihris/data-model/search.html) pages.
