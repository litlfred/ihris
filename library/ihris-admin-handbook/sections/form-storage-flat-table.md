---
title: "Form Storage -- Flat Table"
source: http://open.intrahealth.org/w/index.php?oldid=31407
...
The flat storage mechanism is defined so that, by default, it can read in the cached "hippo\_" tables of form data. If you situation, is not exactly this, then you can take advantage of the options as described in this section.
...
- table: Optional scalar node. The table to use. E.g. my\_table, \`my weird\_table\`, \`some\_other\_database\`.\`table\`. You are responsible for back-tics if required. If it is not set, it determines the table name from the table\_prefix set in the global options.
...
- form\_prepended: Optional scalar node. Defaults to true. If true, it means the form name has been prepended to the id. Otherwise, it assume that the column is just the id. If true, this form is not writable.
...
- table\_prefix: Optional scalar node. The value pre-pended to a form name to give the table name where the data is stored. If not set it is 'hippo\_', the prefix used for the caching of form data to flat tables. For example the person form would use the table 'hippo\_person'.

## Example

To read in the cached hippo\_person tables for the person forms from a cached table we would set:
...
Warning: The id column cannot have 'form\_prepended' set to true for the field to be writable
