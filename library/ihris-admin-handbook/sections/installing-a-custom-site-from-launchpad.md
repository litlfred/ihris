---
title: "Installing a Custom Site from Launchpad"
source: http://open.intrahealth.org/w/index.php?oldid=33650
...
First you should create an account on Launchpad \[1\] if you not have already done so. We will refer to this account as LAUNCHPAD\_USER.

Since we will want to contribute to the code, we will need to create a ssh public key \[2\] on your Ubuntu machine to add to Launchpad:
...
Your key pair is now stored in ~/.ssh/id\_rsa.pub (public key) and ~/.ssh/id\_rsa (private key). Now you need to upload the public portion of your SSH key to Launchpad. To do this, open in your web browser:
...
First we need to make sure the Bazaaar \[3\] (bzr) version control software is installed:

```
 sudo apt-get install bzr bzrtools
```

You may wish to read the five minute tutorial \[4\] at this point. You should also let bzr know how you are:
...
If you wish to make modifications to the CSSC Customizations, you should join the ihris+cssc \[5\] team.
...
To see why we are doing a checkout instead of a branch, read this \[6\]. To make sure everything is working OK, you should make sure the following succeeds:
...
We will create a database called \`cssc\_central\_4\_0\` as follows:
...
$i2ce\_site\_database cssc\_central\_4\_0

$i2ce\_site\_database\_user cssc

$i2ce\_site\_database\_password PASSWORD (the password you set above)
...
- to open a file to edit, type nano the\_file\_name.php on the command line
...
## separate buffer).  You must have configured with
--enable-multibuffer
## for this to work.
##
# set multibuffer
...
# set nofollow
## Don't display the helpful shortcut lists at the bottom of the
screen.
# set nohelp
...
## is changed to here, so any files are inserted from this dir.  A
blank
...
# set view
## The two single-column characters used to display the first
characters
## of tabs and spaces.  187 in ISO 8859-1 (0000BB in Unicode) and 183
in
...
## structure
color brightyellow "\<(class|new|private|public|function|for|foreach|if|while|do|else|els
## control flow
...
##############################################################################
syntax "ml"
".*\.([jrs]?html?|xml|sgml?|lhtml|opml|kdevelop|vcproj|glade|xsd|plist|gcs|dtd|dcl)$"
 "catalog$" "docbook$"
color white "^.+$"
...
We don't use these so I don't have any recommendations. You may want to look here \[7\] for some recommendations.

- gedit Not geared to programming. Installed by default on ubuntu desktop.
- scite \[8\] (sudo apt-get install scite)

Here are some IDEs

- geany A small GUI editor geared to programming (sudo apt-get install geany)
- bluefish \[9\] (sudo apt-get install bluefish)
- eclipse

## References

\[1\] https://launchpad.net/ \[2\] https://help.launchpad.net/YourAccount/CreatingAnSSHKeyPair \[3\] http://bazaar-vcs.org/en/ \[4\] http://doc.bazaar-vcs.org/latest/en/mini-tutorial/index.html \[5\] http://www.launchpad.net/~ihris+cssc \[6\] http://bazaar-vcs.org/CheckoutTutorial \[7\] https://help.ubuntu.com/community/Programming \[8\] http://scintilla.org/SciTE.html \[9\] http://bluefish.openoffice.nl/features.html
