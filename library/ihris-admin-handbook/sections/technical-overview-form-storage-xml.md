---
title: "Technical Overview: Form Storage -- XML"
source: http://open.intrahealth.org/w/index.php?oldid=33554
...
- id: Optional parent node. The data defining how to associate an id &lt;namespace:OBS\_VALUE/&gt;.
...
- attribute: Optional scalar node. The name of the attribute which contains the id. If not set the id is the numeric document order of OBS\_VALUE nodes starting at 1.
...
- form\_prepended: Optional scalar node. Defaults to true. If true, it means the form name (with a |) has been prepended to the id and we need to strip it out to get the id. If ieval is also set, and this is true, the form is stripped before passing to the eval.
