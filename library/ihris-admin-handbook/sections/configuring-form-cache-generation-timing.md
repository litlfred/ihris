---
title: "Configuring Form Cache Generation Timing"
source: http://open.intrahealth.org/w/index.php?oldid=12054
...
/modules/CachedForms/times/background\_time
...
/modules/CachedForms/times/stale\_time
...
/modules/CachedForms/times/fail\_time
...
/modules/CachedForms/times/recache\_time
...
/modules/CachedForms/times/stale\_time\_by\_storage
...
This overrides the stale\_time setting based on the storage mechanism for the form. Some form storage mechanisms may not be changed as often so the cache won't need to be updated as frequently. The values can be for any storage mechanism that you have available in the system.

/modules/CachedForms/times/stale\_time\_by\_form

Default values: person:3

This overrides the stale\_time and stale\_time\_by\_storage settings based on the individual form. Some forms may need to be updated more frequently like the person form since it may be used more frequently.
...
1. Is background\_time greater than 0?

1. If yes, then has background\_time in minutes passed since the last time the process was started?

1. If yes, then is the caching status anything other than "done?"

1. If yes, then has more than fail\_time minutes passed since it started?

1. If yes, then force caching all the forms. 2. If no, then don't run. 2. If no, then cache all the forms based on the stale times for those forms. 2. If no, then don't run. 2. If no, then don't run.
...
1. Is there another process currently "in\_progress?"

1. If yes, then was the command forced?

1. If yes, then force running the caches for each form. 2. If no, then do nothing. 2. If no, then run all the caches for each form.
...
1. Has stale\_time passed since this form was last cached? (Using any of the stale settings mentioned above: stale\_time, stale\_time\_by\_storage or stale\_time\_by form.)

1. If yes, then is the form dirty? (Have any changes been made since the cache was last run?)

1. If yes, then update the cache. 2. If no, then do nothing. 2. If no, was the command forced?

1. If yes, then is the form dirty?

1. If yes, then update the cache. 2. If no, then do nothing. 2. If no, then do nothing.
...
<configurationGroup name="CachedForm_times" path="/modules/CachedForms/times">
  ...
  <configuration name="background_time">
    ...
    <description>The time (in minutes) after which to launch the background page.
      You can disable the launching of the background page by setting
this to be less
      than or equal to 0
    </description>
    <value>0</value>
  ...
  <configuration name="stale_time">
    ...
    <description>The time (in minutes) after which a cached table is consider stale.
      Setting to be less than or equal to zero means that it is always
considered stale.
    </description>
    <value>10</value>
  </configuration>
  <configuration name="fail_time">
    ...
    <description>The time (in minutes) after which generation of a cached table is
consider to have failed</description>
    <status>required:true</status>
    <value>15</value>
  </configuration>
  <configuration name="recache_time">
    <displayName>Fail Time</displayName>
    <description>The time (in minutes) after which the cached table is recached</descript
    <status>required:true</status>
    <value>1440</value>
  </configuration>
  <configuration name='stale_time_by_storage' values='many' type='delimited'>
    <description>The default stale time in minutes for a form based on its storage
mechanism.
      If set, overides the value under times/stale_time.    Setting to
be less than or equal to
  ...
  <configuration name='stale_time_by_form' values='many' type='delimited'>
    <description>The default stale time in minutes for a specific form.  If set,
overides the
      value under times/stale_time_by_storage and times/stale_time.
Setting to be less than or
