...
def parse_details(rows, rec, withheld):
    last = [None, None]
    for r in rows[1:]:
...
def parse_report(text, product):
    ...
    while i < len(lines):
        ...
        if l.startswith("|"):
            ...
            if re.match(r"^UC-[A-Z]+\d+ ", first):
                ...
                parse_details(rows, ctx, withheld)
    ...
    return doc, head, toc_actors, withheld, stats
...
def md_uc(uc, xw):
    ...
    if uc.get("description"):
        L += [uc["description"], ""]
    meta = [("Priority", uc.get("priority")), ("Primary actors", ", ".join(uc.get("primaryActors") or []) or None),
            ("Supporting actors", ", ".join(uc.get("supportingActors") or []) or None), ("Level", uc.get("level")),
            ("Complexity", uc.get("complexity")), ("Status", uc.get("status")), ("Implementation status", uc.get("implementationStatus")),
            ("Release", uc.get("release"))]
...
def write_md(product, doc, head, xw, dangling):
    doc, head, dangling = esc_all(doc), esc_all(head), esc_all(dangling)
    ...
    for p in list(walk(root))[1:]:
        ...
        for r in p["requirements"]:
            ...
            if r.get("description"):
                L += [r["description"], ""]
            meta = [("Priority", r.get("priority")), ("Type", r.get("type")), ("Status", r.get("status")), ("Release", r.get("release"))]
...
def main():
    ...
    for fe in man["files"]:
        ...
        doc, head, toc_actors, withheld, stats = parse_report(text, fe["product"])
        ...
        extras[fe["product"]] = (toc_actors, withheld, stats)
    ...
    for product, d in docs.items():
        toc_actors, withheld, stats = extras[product]
        ...
        rec = {
            ...
            "withheld": [{"field": k, "why": WITHHELD[k], "count": withheld[k]} for k in sorted(withheld)],
            "actors": d["actors"],
            "root": d["root"],
        ...
        with open(os.path.join(OUT, f"{product}.json"), "w", encoding="utf-8") as f:
            ...
            f.write("\n")
        write_md(product, d, heads[product], xw_by, dangling)
    ...
    with open(os.path.join(OUT, "manifest.jsonld"), "w", encoding="utf-8") as f:
        json.dump({"@context": "https://litlfred.github.io/folio-assistant/ns/content/v1.jsonld", "@id": f"{ENTRY}/manifest",
                   ...
                   "contains": [f"{ENTRY}/{p}" for p in PRODUCTS], "provenance": "ingested",
                   "meta": {"source_files": [fe["file"] for fe in man["files"]], "document_class": "use-case-model",
                            "licence": "owner permission, 2026-09-23",
                            "disposition": "ingested: parsed to ihris-use-cases/v1 and Markdown; the .doc files are held in uploads/ (git-ignored)"}},
